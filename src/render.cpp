...
void Game::renderUI(double now) {
    ...
    if (drinkT <= 0 && (weapon == WEAPON_REVOLVER || (weapon == WEAPON_FLARE && flares > 0)))
        DrawCircle(sw / 2, sh / 2, hud(1.5f), {230,220,190,110});
    ...
    {   // inventory, bottom-left; the selected weapon is lit
        ...
        if (coins > 0 || wayOpen())   // doubloons double as your ticket out (ESCAPE_COST to leave)
            hudText(TextFormat("doubloons  ×%d / %d", coins, ESCAPE_COST), hud(16), sh - hud(116), hud(16),
                    wayOpen() ? Color{ 120, 230, 140, 210 } : Color{ 214, 178, 92, 170 });
        hudText(TextFormat("3  almond water  ×%d", almond), hud(16), sh - hud(94), hud(16),
                almond > 0 ? Color{ 150, 190, 235, 170 } : dimc);
        hudText(reloadT > 0 ? "1  revolver  [reloading]"
                    : TextFormat("1  revolver  %d/%d%s", ammo, MAXAMMO, ammo == 0 ? "  · R" : ""),
                    hud(16), sh - hud(50), hud(16), weapon == WEAPON_REVOLVER ? selc : dimc);
        hudText(TextFormat("2  flare  ×%d", flares), hud(16), sh - hud(72), hud(16), weapon == WEAPON_FLARE ? selc : dimc);
        hudText(!deck.carried
