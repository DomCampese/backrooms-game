...
| | |
|---|---|
| ![Level 0](docs/levels/level_0.png) **LEVEL 0** — yellowed office maze, humming fluorescents, mustard carpet. Where the Smilers wait in the dark. | ![Level 1](docs/levels/level_1.png) **LEVEL 1** — cavernous concrete warehouse, sparse lights, loading docks. |
...
The wandering entity — **a Smiler** in most halls, **the Partygoer =)**
...
| `src/entity.h` | the Smiler's state (the state machine runs in `Game::updateEntity`) |
...
- **THE SMILER** — true to the [Backrooms lore](https://backrooms.fandom.com/wiki/Entity_3):
  ...
  wall geometry. Stare too long and it chases. It is slightly slower than your
  sprint. A landed lunge takes a chunk of your health rather than the run;
  health comes back if nothing touches you for a few seconds.
...
- **Flares** — throw one (Q, or left click when selected) and it arcs,
  ...
  in the shader, with a synthesized strike-and-hiss. The Smiler won't come near
...
- **Revolver** — select with 1 (or the mouse wheel). Hold right mouse for iron sights; release to lower the gun. Six rounds, R to reload after lowering the sights
  ...
  that lights the hall. A hit rocks the Smiler back and costs it a
...
- **The pack** — the Red Halls have their own residents, and they don't hunt
  the way the Smiler does. The Smiler hunts by sight: stare at it and it comes. **The
...
- **Exits** — glowing doorways carved into wall runs. A normal one takes you
  ...
  Red Halls instead).
- **Getting out for good** — the doubloons a Smiler leaves when you put it
  ...
  to fight the Smiler to earn your way out at all.
...
- **The Partygoer =)** — LEVEL FUN has its own resident: pale yellow, a
  painted-on smile, a striped party hat. It stalks and chases like the Smiler,
...
`E` spawn the Smiler stalking ahead, `C` force a chase, `H` despawn it,
